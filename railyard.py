...
with tempfile.TemporaryDirectory() as tmpdirname:
    print('Creating Dockerfile in temporary directory: ', tmpdirname)
    ...
    with open(filename) as file:
        ...
        conda_packages = []
        for entry in package['dependencies']['conda']:
            if type(entry)==dict:
        ...
        pip_packages = []
        for entry in package['dependencies']['pip']:
            if type(entry)==dict:
        ...
        labextensions = []
        if 'labextensions' in package['dependencies']:
            for entry in package['dependencies']['labextensions']:
                if type(entry)==dict:
        ...
        with open('Dockerfile', 'w') as file:
            ...
            file.write('\n')
            # Copy files to add
            for addfile in package['dependencies']['addfiles']:
            ...
            file.write(dockerfile_block('ENV',[f'{list(dep.keys())[0]}={list(dep.values())[0]} \\' if i<len(package['dependencies']['env'])-1 else f'{list(dep.keys())[0]}={list(dep.values())[0]}' for i,dep in enumerate(package['dependencies']['env'])]))
            file.write('\n')
            ...
            file.write(dockerfile_block('RUN', ['apt-get update && \\',
                                                ...
                                                'apt-fast update && \\',
                                                f'apt-fast install -yq --no-install-recommends {" ".join(package["dependencies"]["apt"])} && \\',
            ...
            file.write('\n')
            # Extract scripts
            scripts = [item.split('\n') for sublist in [list(script.values()) for script in package['dependencies']['scripts']] for item in sublist]
            for s in scripts:
